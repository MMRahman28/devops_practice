...
## Today's Progress (April 12, 2026)
...
- **Docker Concepts - Part 3**
	**Volume**
	...
	- Recommended mechanism for persisting data generated by Docker container or used by it.
	- Decoupled from container's lifecycle. Deleting container does not affect the volume.

	- Data inside a container is ephemeral.
	- Easier to backup, share between containers and manage.
	- Enforce separation of concern: code + config in the image, stateful data in volumes.

	- What happens to data inside a container when the container is stopped or removed?

	- Data in container's writable layer -> get lost if container is removed, if only stopped writable layer data still available when the container is started again.
	- Data in volumes persist.

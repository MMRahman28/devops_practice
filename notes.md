...
- **Docker Concepts - part 3**

	**Volume**
	- What are Docker Volumes? Why would you use a volume instead of storing data inside the container's filesystem?

	- Docker volumes are recommended mechanism for persisting data generated by or used by container.
	...
	- Data written to the container's writable layer (top layer) get lost on removal. Data in volumes persist. When a container is only stopped (not removed) the writable layer data is still there and will be available when the container is started again.
